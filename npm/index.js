...
class chartgeneratorWrapper {
    ...
    async execute(query, callback) {
        ...
        var requiredParams = ["type","data[labels]","data[datasets][0][label]","data[datasets][0][data]"];
        if (requiredParams.length > 0) {
            for (var i = 0; i < requiredParams.length; i++) {
                if (!query[requiredParams[i]]) {
                    throw new Error(`Required parameter [${requiredParams[i]}] is missing. See documentation: https://docs.apiverve.com/ref/chartgenerator`);
                }
            }
        }
